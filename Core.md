...
- **Fonte de verdade:** Prefira alinhar com `https://github.com/ricardoviannajr/AIConfig`. Para leitura da "Fonte de Verdade Absoluta", utilize **obrigatoriamente** mÃ©todos **Headless/CLI** (como `read_url_content`, `git show origin/master:[file]` ou `gh api`) em vez do Browser Agent.
...
0. **Check de Ambiente (Workflows):** No inÃ­cio de cada sessÃ£o, o Colaborador deve verificar se os workflows globais estÃ£o presentes em `~\.Core\Architecture\global_workflows`. Se ausentes, deve executar `scripts/install-workflows.ps1` do AIConfig para restaurÃ¡-los.
